/**
 * create-text-compare-articles.js (fixed)
...
'best-free-text-comparison-tools-2026': [
...
'online-string-compare-tool-for-developers': [
...
for (const article of articles) {
  ...
  lines.push('    title: "' + article.title.replace(/"/g, '\\"') + '",');
  lines.push('    description: "' + article.description.replace(/"/g, '\\"') + '",');
  lines.push('    url: "https://toolcraftkit.com/blog/' + article.slug + '",');
  lines.push('    siteName: "ToolCraftKit",');
  ...
  lines.push('    canonical: "https://toolcraftkit.com/blog/' + article.slug + '",');
  lines.push('  },');
  lines.push('};');
  lines.push('');
  lines.push(STYLES);
  ...
  lines.push('  return (');
  lines.push('    <div style={s.page}>');
  lines.push(bodyContent[article.slug]);
  lines.push('      <div style={{ marginTop: 48, paddingTop: 24, borderTop: "1px solid #E7E5E4" }}>');
  lines.push('        <p style={{ fontSize: 13, color: "#78716C" }}>');
  lines.push('          Published on ToolCraftKit — <Link href="/blog" style={s.link}>More articles</Link> | <Link href="/tools/text-compare" style={s.link}>Text Compare Tool</Link>');
  lines.push('        </p>');
  lines.push('      </div>');
  lines.push('    </div>');
  lines.push('  );');
