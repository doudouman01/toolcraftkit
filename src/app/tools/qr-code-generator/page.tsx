...
export default function Page() {
  ...
  const s = {
    ...
    btn: { background: canUse ? "#0D9488" : "#A8A29E", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontSize: 14, fontWeight: 600, cursor: canUse ? "pointer" : "not-allowed", opacity: canUse ? 1 : 0.6 } as const,
    btnSm: { background: "#F5F5F4", color: "#1C1917", border: "1px solid #E7E5E4", borderRadius: 6, padding: "6px 14px", fontSize: 13, cursor: canUse ? "pointer" : "not-allowed", opacity: canUse ? 1 : 0.6 } as const,
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        <div style={s.row}>
          {MODES.map((m) => (
            <button key={m.key} onClick={() => { setMode(m.key); setInput(""); }} style={{ ...s.btnSm, background: mode === m.key ? "#0D9488" : "#F5F5F4", color: mode === m.key ? "#fff" : "#1C1917", border: mode === m.key ? "1px solid #0D9488" : "1px solid #E7E5E4", opacity: 1, cursor: "pointer" }}>{m.label}</button>
          ))}
        </div>
          ...
          <>
            ...
            <div style={s.row}>
              ...
              {["WPA", "WEP", "nopass"].map((t) => (
                <button key={t} onClick={() => setWifiType(t)} style={{ ...s.btnSm, background: wifiType === t ? "#6366F1" : "#F5F5F4", color: wifiType === t ? "#fff" : "#1C1917", opacity: 1, cursor: "pointer" }}>{t === "nopass" ? "None" : t}</button>
              ))}
            </div>
          </>
        ) : (
      ...
      {qrDataUrl && (
        <div style={{ ...s.card, textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={() => download("png")} style={s.btn} disabled={!canUse}>Download PNG</button>
            <button onClick={copyImage} style={s.btnSm} disabled={!canUse}>{copied ? "Copied!" : "Copy to Clipboard"}</button>
          </div>
        </div>
      )}
